...
local unlockTrigger = script:FindChildByName("UnlockTier3Enormous"):FindChildByType("Trigger")
...
function PresentBehaviour ()
    if isUnlocked == true then
        ...
        present:SetScale(Vector3.New(10,7,3))
        present:SetRotation(Rotation.New(0,0,90))
        MovePresent(present)
...
function UnlockThisProduction()
    ...
    if isUnlocked == false and points >= 10000 then
        isUnlocked = true
        PresentBehaviour()
    end
end
